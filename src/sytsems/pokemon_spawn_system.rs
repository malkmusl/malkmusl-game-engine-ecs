    ...
    async fn spawn() {
        unsafe {
            let spawn_pokemon_handle = thread::spawn(|| {
                print_info("Starting Spawn Pokemon Handle...", None, None);
                loop {
            ...
            spawn_pokemon_handle.join().expect(&print_error("Unable to start Spawn Pokemon Handle!", None, None));
        }
    }
    ...
    async fn despawn() {
        unsafe {
            ...
            let despawn_pokemon_handle = thread::spawn(|| {
                print_info("Starting Despawn Pokemon Handle...", None, None);
                loop {
            ...
            despawn_pokemon_handle.join().expect(&print_error("Unable to start Despawn Pokemon Handle!", None, None));
        }
    }
}
